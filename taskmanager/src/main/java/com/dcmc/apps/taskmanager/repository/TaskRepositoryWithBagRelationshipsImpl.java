...
public class TaskRepositoryWithBagRelationshipsImpl implements TaskRepositoryWithBagRelationships {
    ...
    public Optional<Task> fetchBagRelationships(Optional<Task> task) {
        return task.map(this::fetchAssignedMembers);
    }
    ...
    public List<Task> fetchBagRelationships(List<Task> tasks) {
        return Optional.of(tasks).map(this::fetchAssignedMembers).orElse(Collections.emptyList());
    }

    Task fetchAssignedMembers(Task result) {
        return entityManager
            .createQuery("select task from Task task left join fetch task.assignedMembers where task.id = :id", Task.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Task> fetchAssignedMembers(List<Task> tasks) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, tasks.size()).forEach(index -> order.put(tasks.get(index).getId(), index));
        List<Task> result = entityManager
            .createQuery("select task from Task task left join fetch task.assignedMembers where task in :tasks", Task.class)
